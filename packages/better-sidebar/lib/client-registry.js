window.__ModuleLoader__.load({
	id: "starpivot/dsh-better-sidebar",
	factory: (require) => {
		var module = { exports: {} };
		...
		var SidebarStore = class {
			...
			*/
			...
			setSession(sessionId) {
				...
				if (sessionId === void 0) this.snapshot = {
					...
					state: void 0,
					prefs: this.prefs,
					treeRefreshTick: this.snapshot.treeRefreshTick
				};
				else {
			...
			*/
			...
			bumpTreeRefresh() {
				this.snapshot = {
					...this.snapshot,
					treeRefreshTick: this.snapshot.treeRefreshTick + 1
				};
				this.notify();
			}
			/**
		...
		*/
		const SIDEBAR_SERVICE_VERSION = "0.13.5";
		/**
		...
		*/
		function loadChunk(name) {
			...
			const task = (async () => {
				...
				if (modules === void 0) throw new Error(`[dsh-better-sidebar] chunk "${name}": client module system not captured at activation`);
		...
		*/
		function relativeTo(cwd, path) {
			const nBase = normalizePath(cwd.replace(/[\\/]+$/, ""));
		...
		*/
		function workspaceRoots(cwd, folders = []) {
			const roots = [];
			const seen = /* @__PURE__ */ new Set();
			const add = (raw) => {
		...
		*/
		function containingRoot(roots, path) {
			let best;
			let bestLength = -1;
			for (const root of roots) {
				if (relativeTo(root, path) === path) continue;
		...
		* @param path - an absolute filesystem path.
		*/
		function relativeToWorkspace(cwd, folders, path) {
			const roots = workspaceRoots(cwd, folders);
			const root = containingRoot(roots, path) ?? roots[0];
			return root === void 0 ? path : relativeTo(root, path);
		}
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Volumes/ExternalData/Projects/dsh-plugins-public/packages/better-sidebar/src/client/sidebar.module.css.mjs
		const css$3 = ".rdMWiq_toggleCluster{z-index:45;flex-direction:row;gap:4px;display:flex;position:fixed;top:3px;right:10px}.rdMWiq_panel:not(.rdMWiq_panelHidden) .rdMWiq_tabBar{padding-right:104px}.rdMWiq_toggleButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;display:flex}.rdMWiq_toggleButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_toggleButton:disabled{opacity:.4;cursor:default}.rdMWiq_panel{z-index:40;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:fixed;top:0;bottom:0;right:0}.rdMWiq_panelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translate(102%)}.rdMWiq_panel[data-dragging]{transition:none}.rdMWiq_panelResize{cursor:col-resize;z-index:2;touch-action:none;width:8px;position:absolute;top:0;bottom:0;left:-4px}.rdMWiq_panelResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.rdMWiq_panelBody{flex:1;min-width:0;min-height:0;display:flex}.rdMWiq_bottomPanel{z-index:40;background:var(--dsw-alias-bg-layer-1);border-top:1px solid var(--dsw-alias-border-l2);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:fixed;bottom:0}.rdMWiq_bottomPanelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translateY(102%)}.rdMWiq_bottomPanel[data-dragging]{transition:none}.rdMWiq_bottomResize{cursor:row-resize;z-index:2;touch-action:none;height:8px;position:absolute;top:-4px;left:0;right:0}.rdMWiq_bottomResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.rdMWiq_bottomClose{z-index:4;width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:3px;right:6px}.rdMWiq_bottomClose:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_bottomPanel .rdMWiq_tabBar{padding-right:40px}body[data-dsh-title-bar-compat] .rdMWiq_toggleCluster{top:calc(var(--dsh-title-bar-strip,40px) + 3px)}body[data-dsh-title-bar-compat] .rdMWiq_panel{padding-top:var(--dsh-title-bar-strip,40px)}.rdMWiq_cornerHandle{left:-6px;bottom:calc(var(--dsh-sidebar-height,0px) + 6px);z-index:2;cursor:nwse-resize;touch-action:none;width:12px;height:12px;position:absolute}.rdMWiq_cornerHandle:hover,.rdMWiq_cornerHandle[data-dragging]{background:var(--dsw-alias-interactive-bg-hover-accent)}.rdMWiq_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.rdMWiq_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_iconButton:disabled{opacity:.4;cursor:default}.rdMWiq_workbench,.rdMWiq_split{flex:1;min-width:0;min-height:0;display:flex}.rdMWiq_splitRow{flex-direction:row}.rdMWiq_splitCol{flex-direction:column}.rdMWiq_splitChild{display:flex;position:relative;overflow:hidden}.rdMWiq_divider{z-index:3;touch-action:none;flex:none;position:relative}.rdMWiq_dividerRow:after,.rdMWiq_dividerCol:after{content:\"\";background:var(--dsw-alias-border-l2);transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);position:absolute}.rdMWiq_dividerRow{cursor:col-resize;width:7px;margin:0 -2px}.rdMWiq_dividerRow:after{width:1px;top:0;bottom:0;left:50%;transform:translate(-50%)}.rdMWiq_dividerCol{cursor:row-resize;height:7px;margin:-2px 0}.rdMWiq_dividerCol:after{height:1px;top:50%;left:0;right:0;transform:translateY(-50%)}.rdMWiq_divider:hover:after,.rdMWiq_dividerActive:after{background:var(--dsw-alias-interactive-bg-hover-accent)}.rdMWiq_pane{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}.rdMWiq_paneDrop{outline:1px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.rdMWiq_dropOverlay{z-index:6;pointer-events:none;background:var(--dsw-alias-interactive-bg-hover-accent);opacity:.5;position:absolute}.rdMWiq_dropLeft{width:25%;top:0;bottom:0;left:0}.rdMWiq_dropRight{width:25%;top:0;bottom:0;right:0}.rdMWiq_dropUp{height:25%;top:0;left:0;right:0}.rdMWiq_dropDown{height:25%;bottom:0;left:0;right:0}.rdMWiq_dropCenter{outline:2px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-2px;background:0 0;inset:25%}.rdMWiq_paneContent{flex-direction:column;flex:1;min-height:0;display:flex;overflow:hidden}.rdMWiq_paneTab{flex-direction:column;flex:1;min-height:0;display:flex}.rdMWiq_paneTabHidden{display:none}.rdMWiq_paneEmptyCards{flex:1;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));align-content:start;gap:8px;min-height:0;padding:12px;display:grid;overflow:hidden}.rdMWiq_paneCard{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;text-align:center;border-radius:8px;flex-direction:column;justify-content:center;align-items:center;gap:6px;padding:12px 8px;display:flex}.rdMWiq_paneCard:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2)}.rdMWiq_paneCard:disabled{opacity:.45;cursor:default}.rdMWiq_tabBar{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:stretch;height:34px;display:flex}.rdMWiq_tabBarDrop{outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.rdMWiq_tabList{scrollbar-width:none;flex:1;min-width:0;display:flex;overflow-x:auto}.rdMWiq_tabList::-webkit-scrollbar{display:none}.rdMWiq_tab{min-width:64px;max-width:160px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);border-right:1px solid var(--dsw-alias-border-l1);cursor:pointer;user-select:none;background:0 0;flex:none;align-items:center;gap:4px;padding:0 4px 0 10px;display:flex}.rdMWiq_tab:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_tabActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.rdMWiq_tabTitle{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.rdMWiq_tabBadge{min-width:16px;height:15px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-brand-primary);border-radius:8px;flex:none;justify-content:center;align-items:center;padding:0 4px;display:inline-flex}.rdMWiq_tabClose{width:18px;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.rdMWiq_tabClose:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_tabBarPlus{background:var(--dsw-alias-bg-layer-1);width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;border:none;border-radius:5px;flex:none;justify-content:center;align-self:center;align-items:center;margin:0 6px;padding:0;display:inline-flex;position:sticky;right:0}.rdMWiq_tabBarPlus:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_explorer{flex-direction:column;flex:1;min-height:0;display:flex}.rdMWiq_explorerHeader{flex:none;justify-content:space-between;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.rdMWiq_explorerRoot{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rdMWiq_explorerBody{flex:1;min-height:0;padding:2px 6px 8px;overflow:hidden auto}.rdMWiq_explorerRow{box-sizing:border-box;width:100%;max-width:100%;height:34px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;white-space:nowrap;animation:rdMWiq_dsh-row-in .15s var(--ds-ease-in-out);background:0 0;border:none;border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}.rdMWiq_explorerRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_explorerDir{font:var(--dsw-font-s-strong-14)}.rdMWiq_explorerHidden{opacity:.45}.rdMWiq_explorerSymlink{color:var(--dsw-alias-label-tertiary);flex:none}.rdMWiq_explorerBroken .rdMWiq_explorerName{color:var(--dsw-alias-state-error-primary)}.rdMWiq_explorerName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}.rdMWiq_explorerRef{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;padding:0 8px;display:none}.rdMWiq_explorerRef:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_explorerRow:hover .rdMWiq_explorerRef,.rdMWiq_explorerRow:focus-within .rdMWiq_explorerRef{display:inline-flex}.rdMWiq_explorerCopied{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.rdMWiq_explorerError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);cursor:default}@keyframes rdMWiq_dsh-row-in{0%{opacity:0}}.rdMWiq_explorerEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.rdMWiq_editor{flex-direction:column;flex:1;min-height:0;display:flex}.rdMWiq_editorHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:6px;padding:4px 8px;display:flex}.rdMWiq_editorTitle{min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.rdMWiq_editorPathInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.rdMWiq_editorPathInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.rdMWiq_editorTreeToggleActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.rdMWiq_editorBody{flex:1;min-height:0;display:flex}.rdMWiq_editorMain{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.rdMWiq_editorTreeDock{border-left:1px solid var(--dsw-alias-border-l1);flex:none;min-height:0;display:flex;position:relative}.rdMWiq_editorTreeResize{cursor:col-resize;touch-action:none;z-index:3;width:6px;position:absolute;top:0;bottom:0;left:0}.rdMWiq_editorTreeResize:hover{background:var(--dsw-alias-border-l2)}.rdMWiq_editorTreePanel{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.rdMWiq_editorTreePanelFull{flex:1}.rdMWiq_editorTreeSearch{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.rdMWiq_editorSearchInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.rdMWiq_editorSearchInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.rdMWiq_editorSearchHint{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:8px 12px}.rdMWiq_editorSearchResult{width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);text-align:left;cursor:pointer;text-overflow:ellipsis;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 8px;display:block;overflow:hidden}.rdMWiq_editorSearchResult:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_editorStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.rdMWiq_editorStatusError{color:var(--dsw-alias-state-error-primary)}.rdMWiq_dirtyDot{background:var(--dsw-alias-state-warn-primary);border-radius:50%;flex:none;width:7px;height:7px}.rdMWiq_editorPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex:1;justify-content:center;align-items:center;padding:16px;display:flex}.rdMWiq_orphanedType{opacity:.7;overflow-wrap:anywhere;margin-top:8px;font-size:12px;display:block}.rdMWiq_editorBinary{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:12px;padding:24px 16px;display:flex}.rdMWiq_editorBinaryNotice{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.rdMWiq_editorDownloadLink{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), border-color var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:6px;align-items:center;gap:6px;padding:6px 14px;text-decoration:none;display:inline-flex}.rdMWiq_editorDownloadLink:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}.rdMWiq_editorError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);padding:12px 16px}.rdMWiq_editorBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.rdMWiq_sandboxStatus{font:var(--dsw-font-xxxs-11);flex:none;align-items:center;gap:8px;padding:4px 10px;display:flex}.rdMWiq_sandboxStatusOn{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border-bottom:1px solid var(--dsw-alias-border-l1)}.rdMWiq_sandboxStatusOff{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent);border-bottom:1px solid color-mix(in srgb, var(--dsw-alias-state-error-primary) 45%, transparent)}.rdMWiq_sandboxDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:6px;height:6px}.rdMWiq_sandboxStatusOff .rdMWiq_sandboxDot{background:var(--dsw-alias-state-error-primary)}.rdMWiq_sandboxStatusText{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.rdMWiq_sandboxAction{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:6px;flex:none;padding:2px 8px}.rdMWiq_sandboxAction:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_editorHtml{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.rdMWiq_browser{flex-direction:column;flex:1;min-height:0;display:flex}.rdMWiq_browserBar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.rdMWiq_browserAddress{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:8px;flex:1;align-items:stretch;min-width:0;height:28px;display:flex;overflow:hidden}.rdMWiq_browserAddress:focus-within{border-color:var(--dsw-alias-border-l2)}.rdMWiq_browserInput{min-width:0;height:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);background:0 0;border:none;border-radius:0;flex:1;padding:0 10px}.rdMWiq_browserInput:focus{outline:none}.rdMWiq_browserActions{border-left:1px solid var(--dsw-alias-border-l1);flex:none;align-items:stretch;display:flex}.rdMWiq_browserAction{width:28px;height:100%;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:0;justify-content:center;align-items:center;padding:0;display:inline-flex}.rdMWiq_browserAction+.rdMWiq_browserAction{border-left:1px solid var(--dsw-alias-border-l1)}.rdMWiq_browserAction:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_browserAction:disabled{opacity:.35;cursor:default}.rdMWiq_browserMessage{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.rdMWiq_browserFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.rdMWiq_browserStart{text-align:center;min-height:0;font:var(--dsw-font-xs-13);color:var(--dsw-alias-label-tertiary);flex:1;justify-content:center;align-items:center;padding:20px;display:flex}.rdMWiq_browserBlocked{text-align:center;min-height:0;color:var(--dsw-alias-state-warn-primary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:6px;padding:24px;display:flex}.rdMWiq_browserBlockedTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary)}.rdMWiq_browserBlockedDesc{max-width:280px;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary)}.rdMWiq_browserBlockedActions{gap:8px;margin-top:6px;display:flex}.rdMWiq_browserBlockedButton{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-11);cursor:pointer;border-radius:6px;padding:4px 12px}.rdMWiq_browserBlockedButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_editorCm{background:0 0;flex:1;min-height:0;overflow:hidden}.rdMWiq_editorCmHidden{display:none}.rdMWiq_editorCm .cm-editor{height:100%}.rdMWiq_editorCm .cm-editor.cm-focused{outline:none}.rdMWiq_editorModeToggle{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;flex:none;align-items:center;gap:2px;padding:2px;display:inline-flex}.rdMWiq_editorModeButton{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;padding:2px 8px}.rdMWiq_editorModeButton:hover{color:var(--dsw-alias-label-primary)}.rdMWiq_editorModeActive{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.rdMWiq_editorImageWrap{flex:1;justify-content:center;align-items:center;min-height:0;padding:12px;display:flex;overflow:auto}.rdMWiq_editorImage{object-fit:contain;max-width:100%;max-height:100%}.rdMWiq_editorMd{min-height:0;font:var(--dsw-font-xs-13);flex:1;padding:10px 14px;overflow-y:auto}.rdMWiq_selectionPopup{z-index:60;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;cursor:pointer;border-radius:6px;align-items:center;padding:0 10px;display:inline-flex;position:fixed;transform:translate(-50%,calc(-100% - 8px))}.rdMWiq_selectionPopup:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_editorPdf{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex}.rdMWiq_editorPdfToolbar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;justify-content:flex-end;padding:6px 8px;display:flex}.rdMWiq_editorPdfStage{flex:1;min-height:0;display:flex;position:relative}.rdMWiq_editorPdfFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.rdMWiq_editorPdfFrameBlocked{pointer-events:none}.rdMWiq_editorPdfDragShield{z-index:4;pointer-events:none;background:0 0;position:absolute;inset:0}.rdMWiq_editorPdfDragShieldActive{pointer-events:auto}body[data-dsh-tab-dragging] .rdMWiq_editorPdfFrame{pointer-events:none!important}body[data-dsh-tab-dragging] .rdMWiq_editorPdfDragShield{pointer-events:auto!important}.rdMWiq_terminalWrap{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex;position:relative}.rdMWiq_terminal{flex:1;min-height:0;padding:6px 4px 6px 8px}.rdMWiq_terminal .xterm{height:100%}.rdMWiq_terminalBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-wrap:wrap;flex:none;align-items:center;gap:8px;padding:3px 10px;display:flex}.rdMWiq_terminalBannerUrl{word-break:break-all;opacity:.85;flex-basis:100%;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.rdMWiq_boundaryError{z-index:50;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;align-items:flex-start;gap:8px;padding:16px;display:flex;position:fixed;top:0;bottom:0;right:0;overflow:auto}.rdMWiq_terminalRetry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;padding:1px 8px}.rdMWiq_terminalRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_terminalDepsBanner{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-direction:column;flex:none;gap:6px;padding:10px;display:flex}.rdMWiq_terminalDepsTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-state-warn-primary)}.rdMWiq_terminalDepsHint{opacity:.9}.rdMWiq_terminalDepsCommandRow{align-items:flex-start;gap:8px;display:flex}.rdMWiq_terminalRepairCommand{white-space:pre-wrap;word-break:break-all;user-select:text;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:4px;flex:1;max-height:160px;margin:0;padding:6px 8px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:1.5;overflow:auto}.rdMWiq_terminalDepsNote{opacity:.85}.rdMWiq_terminalDepsActions{align-items:center;gap:8px;display:flex}.rdMWiq_tabBoundaryError{min-height:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;flex:1;align-items:flex-start;gap:8px;padding:12px 16px;display:flex;overflow:auto}.rdMWiq_git{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.rdMWiq_gitHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.rdMWiq_gitBranchSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 6px}.rdMWiq_gitSection{border-top:1px solid var(--dsw-alias-border-l1)}.rdMWiq_gitSectionHeader{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);text-transform:uppercase;justify-content:space-between;align-items:center;padding:6px 12px 4px;display:flex}.rdMWiq_gitLink{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-brand-primary);cursor:pointer;background:0 0;border:none;padding:0}.rdMWiq_gitLink:hover:not(:disabled){text-decoration:underline}.rdMWiq_gitLink:disabled{opacity:.4;cursor:default}.rdMWiq_gitRow{min-height:34px;animation:rdMWiq_dsh-row-in .15s var(--ds-ease-in-out);border-radius:8px;align-items:center;gap:6px;margin:0 6px;padding:0 8px;display:flex}.rdMWiq_gitRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_gitRowSelected{background:var(--dsw-alias-interactive-bg-active)}.rdMWiq_gitRowMain{cursor:pointer;text-align:left;background:0 0;border:none;flex:1;align-items:center;gap:8px;min-width:0;padding:3px 0;display:flex}.rdMWiq_gitBadge{width:20px;height:16px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;justify-content:center;align-items:center;display:inline-flex}.rdMWiq_gitName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.rdMWiq_gitEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:4px 12px 8px}.rdMWiq_gitPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.rdMWiq_gitError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;padding:8px 12px}.rdMWiq_gitDiff{border-top:1px solid var(--dsw-alias-border-l1);padding:8px}.rdMWiq_gitDiffTab{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.rdMWiq_gitDiffTabHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.rdMWiq_gitDiffTabTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.rdMWiq_gitDiffFile{align-items:baseline;gap:6px;padding:8px 2px 2px;display:flex}.rdMWiq_gitDiffFilePath{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rdMWiq_gitDiffFileOld{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}.rdMWiq_gitDiffFileTag{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:0 6px}.rdMWiq_gitDiffHunk{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);gap:8px;padding:3px 2px;display:flex}.rdMWiq_gitDiffHunkHeader{color:var(--dsw-alias-label-secondary);flex:none}.rdMWiq_gitDiffHunkSection{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rdMWiq_gitDiffLine{font:var(--dsw-font-markdown-code-block-small);white-space:pre-wrap;overflow-wrap:anywhere;align-items:stretch;min-width:0;line-height:20px;display:flex}.rdMWiq_gitDiffNum{text-align:right;width:36px;color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;padding-right:8px}.rdMWiq_gitDiffCode{flex:1;min-width:0;overflow:visible}.rdMWiq_gitDiffCtx{color:var(--dsw-alias-label-primary)}.rdMWiq_gitDiffDel{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent)}.rdMWiq_gitDiffAdd{color:var(--dsw-alias-state-success-primary);background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent)}.rdMWiq_gitDiffMeta{padding-left:2px}.rdMWiq_gitDiffMetaText{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);font-style:italic}.rdMWiq_gitDiffExpand{width:100%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-brand-primary);cursor:pointer;text-align:center;background:0 0;border:none;margin:4px 0;display:block}.rdMWiq_gitDiffExpand:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_gitConfirmDesc{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);white-space:pre-wrap;margin:0}.rdMWiq_gitCommit{border-top:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;padding:8px 12px;display:flex}.rdMWiq_gitCommitInput{flex:1;min-width:0}.rdMWiq_gitCommitButton{background:var(--dsw-alias-button-primary-fill);height:26px;color:var(--dsw-alias-label-primary-inverted);font:var(--dsw-font-xxs-strong-12);cursor:pointer;border:none;border-radius:6px;flex:none;padding:0 12px}.rdMWiq_gitCommitButton:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}.rdMWiq_gitCommitButton:disabled{opacity:.45;cursor:default}.rdMWiq_gitLogRow{cursor:pointer;border-radius:8px;flex-direction:column;gap:2px;padding:5px 12px;display:flex}.rdMWiq_gitLogRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.rdMWiq_gitLogLine1{align-items:baseline;gap:8px;min-width:0;display:flex}.rdMWiq_gitLogHash{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);flex:none}.rdMWiq_gitLogLine2{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;display:flex}.rdMWiq_gitLogRef{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-brand-primary);white-space:nowrap;border-radius:999px;flex:none;padding:0 5px}.rdMWiq_gitLogSubject{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.rdMWiq_gitLogMeta{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.rdMWiq_gitLogMore{border:1px solid var(--dsw-alias-border-l2);width:calc(100% - 24px);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;margin:4px 12px 8px;padding:6px 0;display:block}.rdMWiq_gitLogMore:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_gitLogMore:disabled{opacity:.5;cursor:default}.rdMWiq_producedRow{flex-wrap:wrap;align-items:center;gap:8px;padding:4px 0;display:flex}.rdMWiq_producedLabel{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.rdMWiq_producedChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:200px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;display:inline-flex;overflow:hidden}.rdMWiq_producedChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rdMWiq_producedChip span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rdMWiq_producedMore{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.rdMWiq_toggleButton:focus-visible,.rdMWiq_bottomClose:focus-visible,.rdMWiq_iconButton:focus-visible,.rdMWiq_browserAction:focus-visible,.rdMWiq_tab:focus-visible,.rdMWiq_tabClose:focus-visible,.rdMWiq_tabBarPlus:focus-visible,.rdMWiq_paneCard:focus-visible,.rdMWiq_explorerRow:focus-visible,.rdMWiq_explorerRef:focus-visible,.rdMWiq_gitRowMain:focus-visible,.rdMWiq_gitLink:focus-visible,.rdMWiq_gitCommitButton:focus-visible,.rdMWiq_gitLogRow:focus-visible,.rdMWiq_gitLogMore:focus-visible,.rdMWiq_gitDiffExpand:focus-visible,.rdMWiq_terminalRetry:focus-visible,.rdMWiq_editorModeButton:focus-visible,.rdMWiq_editorDownloadLink:focus-visible,.rdMWiq_editorPptxButton:focus-visible,.rdMWiq_editorDocxZoomRange:focus-visible{outline:2px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}@media (prefers-reduced-motion:reduce){.rdMWiq_panel,.rdMWiq_panelHidden,.rdMWiq_bottomPanel,.rdMWiq_bottomPanelHidden,.rdMWiq_toggleCluster,.rdMWiq_toggleButton,.rdMWiq_tab,.rdMWiq_tabBarPlus,.rdMWiq_paneCard,.rdMWiq_explorerRow,.rdMWiq_gitRow,.rdMWiq_divider,.rdMWiq_dividerRow:after,.rdMWiq_dividerCol:after{transition:none;animation:none}}@media (width<=767px){.rdMWiq_panel:not(.rdMWiq_panelHidden) .rdMWiq_tabBar{padding-right:40px}.rdMWiq_tab{min-width:48px;max-width:128px}}";
		const tagId$3 = "starpivot/dsh-better-sidebar/sidebar.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$3) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "starpivot/dsh-better-sidebar";
			tag.dataset.pluginCss = tagId$3;
			tag.textContent = css$3;
			document.head.appendChild(tag);
		}
		var sidebar_module_css_default = {
			"editorTreeDock": "rdMWiq_editorTreeDock",
			"gitRowSelected": "rdMWiq_gitRowSelected",
			"gitDiffMetaText": "rdMWiq_gitDiffMetaText",
			"gitSectionHeader": "rdMWiq_gitSectionHeader",
			"editorSearchInput": "rdMWiq_editorSearchInput",
			"editorPdfFrame": "rdMWiq_editorPdfFrame",
			"bottomPanelHidden": "rdMWiq_bottomPanelHidden",
			"dividerActive": "rdMWiq_dividerActive",
			"tabList": "rdMWiq_tabList",
			"gitDiffTab": "rdMWiq_gitDiffTab",
			"paneDrop": "rdMWiq_paneDrop",
			"browserBar": "rdMWiq_browserBar",
			"gitDiffCtx": "rdMWiq_gitDiffCtx",
			"iconButton": "rdMWiq_iconButton",
			"terminal": "rdMWiq_terminal",
			"explorerRow": "rdMWiq_explorerRow",
			"terminalRetry": "rdMWiq_terminalRetry",
			"gitDiffHunk": "rdMWiq_gitDiffHunk",
			"gitLogLine1": "rdMWiq_gitLogLine1",
			"editorBinaryNotice": "rdMWiq_editorBinaryNotice",
			"explorerRoot": "rdMWiq_explorerRoot",
			"explorerRef": "rdMWiq_explorerRef",
			"dropRight": "rdMWiq_dropRight",
			"tabBar": "rdMWiq_tabBar",
			"pane": "rdMWiq_pane",
			"sandboxStatusText": "rdMWiq_sandboxStatusText",
			"editorPdfStage": "rdMWiq_editorPdfStage",
			"terminalDepsTitle": "rdMWiq_terminalDepsTitle",
			"editorPathInput": "rdMWiq_editorPathInput",
			"editorModeButton": "rdMWiq_editorModeButton",
			"splitChild": "rdMWiq_splitChild",
			"editorError": "rdMWiq_editorError",
			"paneCard": "rdMWiq_paneCard",
			"gitName": "rdMWiq_gitName",
			"tabActive": "rdMWiq_tabActive",
			"browserBlocked": "rdMWiq_browserBlocked",
			"editorTreePanel": "rdMWiq_editorTreePanel",
			"tab": "rdMWiq_tab",
			"tabBarPlus": "rdMWiq_tabBarPlus",
			"panelResize": "rdMWiq_panelResize",
			"editorImageWrap": "rdMWiq_editorImageWrap",
			"terminalBanner": "rdMWiq_terminalBanner",
			"editorTitle": "rdMWiq_editorTitle",
			"explorerDir": "rdMWiq_explorerDir",
			"paneTab": "rdMWiq_paneTab",
			"editorMain": "rdMWiq_editorMain",
			"browserBlockedButton": "rdMWiq_browserBlockedButton",
			"editorCm": "rdMWiq_editorCm",
			"boundaryError": "rdMWiq_boundaryError",
			"terminalDepsCommandRow": "rdMWiq_terminalDepsCommandRow",
			"gitDiffExpand": "rdMWiq_gitDiffExpand",
			"gitCommitButton": "rdMWiq_gitCommitButton",
			"tabClose": "rdMWiq_tabClose",
			"browserMessage": "rdMWiq_browserMessage",
			"editorTreeResize": "rdMWiq_editorTreeResize",
			"editorBanner": "rdMWiq_editorBanner",
			"editorHtml": "rdMWiq_editorHtml",
			"gitLogLine2": "rdMWiq_gitLogLine2",
			"editorStatus": "rdMWiq_editorStatus",
			"editorPdfFrameBlocked": "rdMWiq_editorPdfFrameBlocked",
			"editorTreeToggleActive": "rdMWiq_editorTreeToggleActive",
			"browserFrame": "rdMWiq_browserFrame",
			"gitDiffDel": "rdMWiq_gitDiffDel",
			"gitRow": "rdMWiq_gitRow",
			"gitDiffTabTitle": "rdMWiq_gitDiffTabTitle",
			"terminalBannerUrl": "rdMWiq_terminalBannerUrl",
			"producedMore": "rdMWiq_producedMore",
			"producedRow": "rdMWiq_producedRow",
			"browserAction": "rdMWiq_browserAction",
			"bottomResizeActive": "rdMWiq_bottomResizeActive",
			"gitDiffTabHeader": "rdMWiq_gitDiffTabHeader",
			"editorBody": "rdMWiq_editorBody",
			"editorPdf": "rdMWiq_editorPdf",
			"browser": "rdMWiq_browser",
			"editorHeader": "rdMWiq_editorHeader",
			"orphanedType": "rdMWiq_orphanedType",
			"browserInput": "rdMWiq_browserInput",
			"panel": "rdMWiq_panel",
			"editorPdfDragShield": "rdMWiq_editorPdfDragShield",
			"gitLogMore": "rdMWiq_gitLogMore",
			"dropOverlay": "rdMWiq_dropOverlay",
			"producedChip": "rdMWiq_producedChip",
			"gitCommitInput": "rdMWiq_gitCommitInput",
			"gitDiffCode": "rdMWiq_gitDiffCode",
			"editorDocxZoomRange": "rdMWiq_editorDocxZoomRange",
			"toggleButton": "rdMWiq_toggleButton",
			"browserStart": "rdMWiq_browserStart",
			"terminalDepsHint": "rdMWiq_terminalDepsHint",
			"explorer": "rdMWiq_explorer",
			"terminalDepsNote": "rdMWiq_terminalDepsNote",
			"dropDown": "rdMWiq_dropDown",
			"git": "rdMWiq_git",
			"gitBranchSelect": "rdMWiq_gitBranchSelect",
			"editorPdfDragShieldActive": "rdMWiq_editorPdfDragShieldActive",
			"toggleCluster": "rdMWiq_toggleCluster",
			"editorModeToggle": "rdMWiq_editorModeToggle",
			"cornerHandle": "rdMWiq_cornerHandle",
			"gitEmpty": "rdMWiq_gitEmpty",
			"editorTreePanelFull": "rdMWiq_editorTreePanelFull",
			"selectionPopup": "rdMWiq_selectionPopup",
			"gitError": "rdMWiq_gitError",
			"tabTitle": "rdMWiq_tabTitle",
			"editorDownloadLink": "rdMWiq_editorDownloadLink",
			"gitDiff": "rdMWiq_gitDiff",
			"gitPlaceholder": "rdMWiq_gitPlaceholder",
			"gitDiffLine": "rdMWiq_gitDiffLine",
			"dividerCol": "rdMWiq_dividerCol",
			"editorModeActive": "rdMWiq_editorModeActive",
			"splitCol": "rdMWiq_splitCol",
			"editorTreeSearch": "rdMWiq_editorTreeSearch",
			"gitDiffFilePath": "rdMWiq_gitDiffFilePath",
			"editorPptxButton": "rdMWiq_editorPptxButton",
			"divider": "rdMWiq_divider",
			"panelHidden": "rdMWiq_panelHidden",
			"bottomClose": "rdMWiq_bottomClose",
			"workbench": "rdMWiq_workbench",
			"browserBlockedTitle": "rdMWiq_browserBlockedTitle",
			"gitLogRef": "rdMWiq_gitLogRef",
			"explorerBody": "rdMWiq_explorerBody",
			"explorerBroken": "rdMWiq_explorerBroken",
			"sandboxStatusOn": "rdMWiq_sandboxStatusOn",
			"dirtyDot": "rdMWiq_dirtyDot",
			"editorPdfToolbar": "rdMWiq_editorPdfToolbar",
			"gitHeader": "rdMWiq_gitHeader",
			"sandboxStatusOff": "rdMWiq_sandboxStatusOff",
			"paneContent": "rdMWiq_paneContent",
			"paneTabHidden": "rdMWiq_paneTabHidden",
			"terminalWrap": "rdMWiq_terminalWrap",
			"gitLogSubject": "rdMWiq_gitLogSubject",
			"explorerHidden": "rdMWiq_explorerHidden",
			"explorerEmpty": "rdMWiq_explorerEmpty",
			"gitConfirmDesc": "rdMWiq_gitConfirmDesc",
			"editorImage": "rdMWiq_editorImage",
			"gitDiffFileOld": "rdMWiq_gitDiffFileOld",
			"explorerCopied": "rdMWiq_explorerCopied",
			"editorBinary": "rdMWiq_editorBinary",
			"tabBoundaryError": "rdMWiq_tabBoundaryError",
			"tabBadge": "rdMWiq_tabBadge",
			"gitBadge": "rdMWiq_gitBadge",
			"explorerHeader": "rdMWiq_explorerHeader",
			"gitLink": "rdMWiq_gitLink",
			"gitLogMeta": "rdMWiq_gitLogMeta",
			"gitRowMain": "rdMWiq_gitRowMain",
			"splitRow": "rdMWiq_splitRow",
			"explorerSymlink": "rdMWiq_explorerSymlink",
			"panelBody": "rdMWiq_panelBody",
			"explorerError": "rdMWiq_explorerError",
			"editorPlaceholder": "rdMWiq_editorPlaceholder",
			"editorCmHidden": "rdMWiq_editorCmHidden",
			"split": "rdMWiq_split",
			"terminalRepairCommand": "rdMWiq_terminalRepairCommand",
			"browserActions": "rdMWiq_browserActions",
			"tabBarDrop": "rdMWiq_tabBarDrop",
			"producedLabel": "rdMWiq_producedLabel",
			"dividerRow": "rdMWiq_dividerRow",
			"sandboxDot": "rdMWiq_sandboxDot",
			"dsh-row-in": "rdMWiq_dsh-row-in",
			"gitDiffNum": "rdMWiq_gitDiffNum",
			"browserBlockedActions": "rdMWiq_browserBlockedActions",
			"editorStatusError": "rdMWiq_editorStatusError",
			"dropLeft": "rdMWiq_dropLeft",
			"panelResizeActive": "rdMWiq_panelResizeActive",
			"editorMd": "rdMWiq_editorMd",
			"terminalDepsBanner": "rdMWiq_terminalDepsBanner",
			"sandboxAction": "rdMWiq_sandboxAction",
			"explorerName": "rdMWiq_explorerName",
			"gitDiffMeta": "rdMWiq_gitDiffMeta",
			"gitDiffFileTag": "rdMWiq_gitDiffFileTag",
			"gitCommit": "rdMWiq_gitCommit",
			"gitLogHash": "rdMWiq_gitLogHash",
			"dropUp": "rdMWiq_dropUp",
			"terminalDepsActions": "rdMWiq_terminalDepsActions",
			"editor": "rdMWiq_editor",
			"dropCenter": "rdMWiq_dropCenter",
			"editorSearchHint": "rdMWiq_editorSearchHint",
			"editorSearchResult": "rdMWiq_editorSearchResult",
			"sandboxStatus": "rdMWiq_sandboxStatus",
			"gitSection": "rdMWiq_gitSection",
			"gitDiffHunkHeader": "rdMWiq_gitDiffHunkHeader",
			"bottomResize": "rdMWiq_bottomResize",
			"gitDiffHunkSection": "rdMWiq_gitDiffHunkSection",
			"gitDiffAdd": "rdMWiq_gitDiffAdd",
			"gitDiffFile": "rdMWiq_gitDiffFile",
			"gitLogRow": "rdMWiq_gitLogRow",
			"browserAddress": "rdMWiq_browserAddress",
			"browserBlockedDesc": "rdMWiq_browserBlockedDesc",
			"bottomPanel": "rdMWiq_bottomPanel",
			"paneEmptyCards": "rdMWiq_paneEmptyCards"
		};
		//#endregion
		...
		*/
		const ROOT_FOLD_SEEDED = "__dsh-sidebar-root-fold__";
		/** Whether a workspace root row is currently expanded. */
		function isRootExpanded(expanded, root) {
			return expanded.includes("__dsh-sidebar-root-fold__") ? expanded.includes(root) : true;
		}
		/**
		...
		*/
		function nextExpandedForRoot(expanded, roots, root) {
			if (!expanded.includes("__dsh-sidebar-root-fold__")) return [ROOT_FOLD_SEEDED, ...roots.filter((item) => item !== root)];
			return expanded.includes(root) ? expanded.filter((item) => item !== root) : [...expanded, root];
		}
		function FileTree(props) {
			...
			(0, react.useEffect)(() => {
				...
				for (const root of workspaceRoots(cwd, folders)) if (isRootExpanded(expanded, root)) loadDir(root);
				for (const dir of expanded) if (dir !== "__dsh-sidebar-root-fold__") loadDir(dir);
			}, [
				rootsKey,
				cwd,
				folders,
			...
			const renderRoot = (root) => {
				...
				}), isOpen && data[root] !== void 0 && renderLevel(root, 1)] }, root);
			};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [roots.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: sidebar_module_css_default.explorerEmpty,
					children: t("noSession")
				}) : roots.map((root) => renderRoot(root)), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
					open: rowMenu !== null,
					onClose: () => {
						setRowMenu(null);
					},
					...
					onSelect: (id) => {
						...
						copyPath(id === "relative" ? relativeToWorkspace(cwd, folders, target.path) : target.path, target.path);
					},
					portal: true,
					align: "start",
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					children: [
						...
						error === null && results !== null && results.matches.map((rel) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							title: rel,
							onClick: () => {
								onOpenFile(isAbsolutePath(rel) ? rel : resolveSidebarPath(cwd, rel));
							},
							children: rel
						}, rel)),
		...
		*  open and file tabs default closed. */
		...
		function EditorHost(props) {
			...
			const treeRefreshTick = (0, react.useSyncExternalStore)((0, react.useCallback)((callback) => store.subscribe(callback), [store]), (0, react.useCallback)(() => store.getSnapshot().treeRefreshTick, [store]));
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Volumes/ExternalData/Projects/dsh-plugins-public/packages/better-sidebar/src/client/SubagentView.module.css.mjs
		const css$2 = ".rKH_-G_subagent{flex-direction:column;flex:1;min-height:0;display:flex}.rKH_-G_subagentHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.rKH_-G_subagentTitle{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.rKH_-G_subagentCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.rKH_-G_subagentRefresh{width:24px;height:24px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;display:inline-flex}.rKH_-G_subagentRefresh:hover{background:var(--dsw-alias-interactive-bg-hover)}.rKH_-G_subagentBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}.rKH_-G_subagentRow{box-sizing:border-box;width:100%;min-height:50px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;align-items:flex-start;gap:8px;padding:7px 8px 7px 11px;display:flex;position:relative}.rKH_-G_subagentRow:hover,.rKH_-G_subagentRow:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.rKH_-G_subagentRowActive,.rKH_-G_subagentRowActive:hover,.rKH_-G_subagentRowActive:focus-visible{background:var(--dsw-alias-interactive-bg-active)}.rKH_-G_subagentRowDisabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.rKH_-G_subagentRowDisabled:hover{background:0 0}.rKH_-G_subagentRowLoading{cursor:default}.rKH_-G_subagentDot{margin-top:4px}.rKH_-G_subagentContent{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.rKH_-G_subagentLabel,.rKH_-G_subagentSecondary{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rKH_-G_subagentLabel{color:inherit;font-weight:400}.rKH_-G_subagentSecondary{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.rKH_-G_subagentLive{min-width:0;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);align-items:baseline;gap:4px;display:flex;overflow:hidden}.rKH_-G_subagentLiveTool{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);flex:none}.rKH_-G_subagentLiveArgs{min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.rKH_-G_subagentLiveText{-webkit-line-clamp:2;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary);-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.rKH_-G_subagentNode{min-width:0;position:relative}.rKH_-G_subagentChildren{margin-left:18px;padding-left:4px;position:relative}.rKH_-G_subagentChildren:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);height:26px;position:absolute;top:-26px;left:0}.rKH_-G_subagentChildren[aria-busy=true]:before{content:none}.rKH_-G_subagentChildren>.rKH_-G_subagentNode:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);position:absolute;top:0;bottom:0;left:-4px}.rKH_-G_subagentChildren>.rKH_-G_subagentNode:last-child:before{height:17px;bottom:auto}.rKH_-G_subagentChildren>.rKH_-G_subagentNode>.rKH_-G_subagentRow:before{content:\"\";border-top:1px solid var(--dsw-alias-border-l2);width:14px;position:absolute;top:16px;left:-4px}.rKH_-G_subagentEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;gap:2px;padding:16px;display:flex}.rKH_-G_subagentEmptyHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-dimmed)}.rKH_-G_subagentError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;display:flex}.rKH_-G_subagentErrorRetry{height:24px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;align-items:center;gap:4px;padding:0 8px;display:inline-flex}.rKH_-G_subagentErrorRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.rKH_-G_jobs{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:8px}.rKH_-G_jobsHeader{align-items:center;gap:8px;height:26px;padding:0 2px;display:flex}.rKH_-G_jobsTitle{min-width:0;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.rKH_-G_jobsCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.rKH_-G_jobsList{flex-direction:column;gap:2px;margin:0;padding:0;list-style:none;display:flex}.rKH_-G_jobsRow{border-radius:8px;align-items:center;gap:4px;display:flex}.rKH_-G_jobsRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.rKH_-G_jobsRowSettled{opacity:.8}.rKH_-G_jobsRowSelected,.rKH_-G_jobsRowSelected:hover{background:var(--dsw-alias-interactive-bg-active)}.rKH_-G_jobsRowMain{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;flex:1;align-items:flex-start;gap:8px;padding:6px 8px 6px 11px;display:flex}.rKH_-G_jobsRowMain:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.rKH_-G_jobsDot{margin-top:5px}.rKH_-G_jobsContent{flex-direction:column;gap:1px;min-width:0;display:flex}.rKH_-G_jobsLabelLine{align-items:center;gap:6px;min-width:0;display:flex}.rKH_-G_jobsKind{text-overflow:ellipsis;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);max-width:90px;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;padding:0 5px;line-height:14px;overflow:hidden}.rKH_-G_jobsLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.rKH_-G_jobsSecondary{text-overflow:ellipsis;white-space:nowrap;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);overflow:hidden}.rKH_-G_jobsKill{width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;margin-right:4px;display:inline-flex}.rKH_-G_jobsKill:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);color:var(--dsw-alias-state-error-primary)}.rKH_-G_jobsKillArmed,.rKH_-G_jobsKillArmed:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);width:auto;height:20px;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;padding:0 8px}.rKH_-G_jobsKill:disabled{opacity:.5;cursor:default}.rKH_-G_jobsKillError{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-error-primary);flex:none;margin-right:4px}.rKH_-G_jobsPane{z-index:1;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;margin-top:4px;position:sticky;bottom:0;overflow:hidden;box-shadow:0 -6px 12px -8px #00000059}.rKH_-G_jobsPaneHeader{border-bottom:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;display:flex}.rKH_-G_jobsPaneDot{flex:none}.rKH_-G_jobsPaneLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.rKH_-G_jobsPaneStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.rKH_-G_jobsPaneClose{width:20px;height:20px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:5px;flex:none;justify-content:center;align-items:center;display:inline-flex}.rKH_-G_jobsPaneClose:hover{background:var(--dsw-alias-interactive-bg-hover)}.rKH_-G_jobsPanePre{max-height:200px;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;margin:0;padding:6px 10px;line-height:1.5;overflow:auto}.rKH_-G_jobsPaneHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);padding:8px 10px}.rKH_-G_jobsPaneError{color:var(--dsw-alias-state-error-primary)}";
		const tagId$2 = "starpivot/dsh-better-sidebar/SubagentView.module.css";
		...
		var SubagentView_module_css_default = {
			"jobsRowSelected": "rKH_-G_jobsRowSelected",
			"jobsDot": "rKH_-G_jobsDot",
			"jobsContent": "rKH_-G_jobsContent",
			"subagentRowActive": "rKH_-G_subagentRowActive",
			"subagentNode": "rKH_-G_subagentNode",
			"jobsKill": "rKH_-G_jobsKill",
			"subagentLiveText": "rKH_-G_subagentLiveText",
			"jobsRowSettled": "rKH_-G_jobsRowSettled",
			"jobsKillError": "rKH_-G_jobsKillError",
			"subagentHeader": "rKH_-G_subagentHeader",
			"jobsLabel": "rKH_-G_jobsLabel",
			"jobsPaneHeader": "rKH_-G_jobsPaneHeader",
			"jobsPaneDot": "rKH_-G_jobsPaneDot",
			"jobsRow": "rKH_-G_jobsRow",
			"jobsPaneLabel": "rKH_-G_jobsPaneLabel",
			"jobsPaneClose": "rKH_-G_jobsPaneClose",
			"jobsPanePre": "rKH_-G_jobsPanePre",
			"jobsSecondary": "rKH_-G_jobsSecondary",
			"jobsPaneError": "rKH_-G_jobsPaneError",
			"jobsPane": "rKH_-G_jobsPane",
			"subagentContent": "rKH_-G_subagentContent",
			"subagentLiveArgs": "rKH_-G_subagentLiveArgs",
			"subagentChildren": "rKH_-G_subagentChildren",
			"jobsPaneStatus": "rKH_-G_jobsPaneStatus",
			"jobsList": "rKH_-G_jobsList",
			"subagentLiveTool": "rKH_-G_subagentLiveTool",
			"subagentRowDisabled": "rKH_-G_subagentRowDisabled",
			"subagentDot": "rKH_-G_subagentDot",
			"jobsRowMain": "rKH_-G_jobsRowMain",
			"subagentRowLoading": "rKH_-G_subagentRowLoading",
			"subagentTitle": "rKH_-G_subagentTitle",
			"subagentLabel": "rKH_-G_subagentLabel",
			"subagentSecondary": "rKH_-G_subagentSecondary",
			"subagentLive": "rKH_-G_subagentLive",
			"subagentEmpty": "rKH_-G_subagentEmpty",
			"jobsHeader": "rKH_-G_jobsHeader",
			"jobsCount": "rKH_-G_jobsCount",
			"jobs": "rKH_-G_jobs",
			"jobsTitle": "rKH_-G_jobsTitle",
			"jobsKillArmed": "rKH_-G_jobsKillArmed",
			"subagentBody": "rKH_-G_subagentBody",
			"subagentEmptyHint": "rKH_-G_subagentEmptyHint",
			"subagent": "rKH_-G_subagent",
			"subagentCount": "rKH_-G_subagentCount",
			"subagentErrorRetry": "rKH_-G_subagentErrorRetry",
			"jobsPaneHint": "rKH_-G_jobsPaneHint",
			"subagentError": "rKH_-G_subagentError",
			"jobsKind": "rKH_-G_jobsKind",
			"jobsLabelLine": "rKH_-G_jobsLabelLine",
			"subagentRefresh": "rKH_-G_subagentRefresh",
			"subagentRow": "rKH_-G_subagentRow"
		};
		//#endregion
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: sidebar_module_css_default.browserAddress,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									...
									className: sidebar_module_css_default.browserActions,
									role: "group",
									"aria-label": t("browserActions"),
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: sidebar_module_css_default.browserAction,
			...
			return [
				{
					...
					component: ({ ctx, store, scope, tab, expanded, onToggleDir, onReferenceFile, folders }) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(EditorHost, {
						ctx,
						store,
						scope,
						tab,
		...
		* (mirror of the terminal view's own cap; the loop restarts on session switch). */
		...
		const EMPTY_WORKSPACE_LIST = { items: [] };
		...
		* entirely — re-enabling them is the settings page's job. */
		...
		function Sidebar(props) {
			...
			const overlayOpen = (0, react.useSyncExternalStore)((0, react.useCallback)((onStoreChange) => {
			...
			*/
			...
			const [fetchedFolders, setFetchedFolders] = (0, react.useState)([]);
			(0, react.useEffect)(() => {
				...
				api.sessionCwd({
					...
					if (cancelled) return;
					if (summaryCwd === void 0) setFetchedCwd(result.cwd);
			...
			const workspaceList = (0, react.useSyncExternalStore)((0, react.useMemo)(() => {
				const list = ctx.workspaces?.list;
				if (list === void 0) return (_callback) => () => {};
				return (callback) => list.subscribe(callback);
			}, [ctx]), (0, react.useCallback)(() => ctx.workspaces?.list?.getSnapshot() ?? EMPTY_WORKSPACE_LIST, [ctx]));
			const folders = (0, react.useMemo)(() => {
				if (sessionId === void 0) return [];
				const owned = workspaceList.items.find((item) => item.sessionIds.includes(sessionId));
				if (owned !== void 0) return [...owned.folders];
				if (cwd === void 0) return fetchedFolders;
				const byPath = workspaceList.items.find((item) => item.path === cwd);
				return byPath === void 0 ? fetchedFolders : [...byPath.folders];
			}, [
				workspaceList,
				sessionId,
				cwd,
			...
			*/
				...
				onToggleDir: (path) => {
					store.reduce((s) => {
						...
						if (roots.includes(path)) return {
							...s,
							expanded: nextExpandedForRoot(s.expanded, roots, path)
						};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				!overlayOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: sidebar_module_css_default.toggleCluster,
					children: [
						state.panelOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
							label: t("refresh"),
							side: "bottom",
							delayMs: 500,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Volumes/ExternalData/Projects/dsh-plugins-public/packages/better-sidebar/src/client/SideCardSection.module.css.mjs
		const css$1 = "._6Du3AG_section{flex-direction:column;gap:14px;width:100%;max-width:760px;display:flex}._6Du3AG_intro{color:var(--dsw-alias-label-tertiary);margin:0;padding:0 2px;font-size:13px;line-height:20px}._6Du3AG_group{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:16px;flex-direction:column;flex:none;gap:8px;padding:18px 20px 20px;display:flex}._6Du3AG_groupHeading{color:var(--dsw-alias-label-primary);align-items:baseline;gap:7px;padding:0 2px 6px;font-size:13px;font-weight:600;line-height:20px;display:flex}._6Du3AG_count{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;font-size:12px;font-weight:400;line-height:18px}._6Du3AG_grid{grid-template-columns:repeat(auto-fill,minmax(168px,1fr));gap:10px;display:grid}._6Du3AG_card{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:12px;flex-direction:column;transition:background .12s,border-color .12s;display:flex;position:relative}._6Du3AG_card:not(._6Du3AG_cardOn):hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-label-dimmed)}._6Du3AG_cardOn{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-interactive-bg-active)}._6Du3AG_cardMain{border-radius:inherit;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;flex-direction:column;gap:6px;padding:12px;display:flex}._6Du3AG_cardMain:focus-visible,._6Du3AG_cardGear:focus-visible,._6Du3AG_rowGear:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}._6Du3AG_cardTop{align-items:center;gap:8px;min-width:0;display:flex}._6Du3AG_cardIconChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);width:28px;height:28px;color:var(--dsw-alias-label-tertiary);border-radius:8px;flex:none;justify-content:center;align-items:center;display:inline-flex}._6Du3AG_cardOn ._6Du3AG_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);background:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 12%, transparent);color:var(--dsw-alias-button-primary-fill)}._6Du3AG_cardTitle{min-width:0;color:var(--dsw-alias-label-secondary);white-space:nowrap;text-overflow:ellipsis;flex:1;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}._6Du3AG_cardOn ._6Du3AG_cardTitle{color:var(--dsw-alias-label-primary)}._6Du3AG_cardCheck{background:var(--dsw-alias-button-primary-fill);width:16px;height:16px;color:var(--dsw-alias-bg-layer-3);border-radius:50%;flex:none;justify-content:center;align-items:center;display:inline-flex}._6Du3AG_cardDesc{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:11px;line-height:16px;overflow:hidden}._6Du3AG_addCard{border-style:dashed;border-color:var(--dsw-alias-border-l2);text-align:left;align-items:flex-start;padding:12px}._6Du3AG_addCard:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}._6Du3AG_addCard:hover ._6Du3AG_cardTitle{color:var(--dsw-alias-label-primary)}._6Du3AG_addCard:hover ._6Du3AG_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);color:var(--dsw-alias-button-primary-fill)}._6Du3AG_addCard:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}._6Du3AG_cardOn ._6Du3AG_cardDesc{color:var(--dsw-alias-label-secondary)}._6Du3AG_cardWithGear ._6Du3AG_cardDesc{padding-right:30px}._6Du3AG_cardGear{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:50%;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:46px;right:12px}._6Du3AG_cardGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}._6Du3AG_rowGear{width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}._6Du3AG_rowGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}._6Du3AG_row{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:12px 2px;display:flex}._6Du3AG_row:last-child{border-bottom:none}._6Du3AG_rowText{flex-direction:column;gap:4px;min-width:0;display:flex}._6Du3AG_title{color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px}._6Du3AG_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}._6Du3AG_switch{cursor:pointer;flex:none;display:inline-flex;position:relative}._6Du3AG_switchInput{opacity:0;width:1px;height:1px;margin:0;position:absolute}._6Du3AG_switchTrack{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:center;width:36px;height:20px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}._6Du3AG_switchThumb{background:var(--dsw-alias-label-secondary);border-radius:50%;width:14px;height:14px;transition:transform .15s,background .15s;display:block}._6Du3AG_switch:hover ._6Du3AG_switchTrack{border-color:var(--dsw-alias-label-dimmed)}._6Du3AG_switchInput:checked+._6Du3AG_switchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}._6Du3AG_switchInput:checked+._6Du3AG_switchTrack ._6Du3AG_switchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(16px)}._6Du3AG_switchInput:focus-visible+._6Du3AG_switchTrack{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}._6Du3AG_control{flex:none;align-items:center;gap:6px;display:flex}._6Du3AG_percentInput{width:76px}._6Du3AG_typedInput{width:200px}._6Du3AG_typedInputNumber{width:76px}._6Du3AG_selectAnchor{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);max-width:220px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:8px;align-items:center;gap:6px;padding:4px 8px;font-size:13px;line-height:20px;display:flex}._6Du3AG_selectAnchor:hover{border-color:var(--dsw-alias-label-dimmed)}._6Du3AG_selectAnchorIcon{flex:none;display:inline-flex}._6Du3AG_selectAnchorText{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._6Du3AG_selectOption{align-items:center;gap:10px;min-width:200px;display:flex}._6Du3AG_selectOptionIcon{color:var(--dsw-alias-label-secondary);flex:none;display:inline-flex}._6Du3AG_selectOptionText{flex-direction:column;min-width:0;display:flex}._6Du3AG_suffix{color:var(--dsw-alias-label-secondary);font-size:14px;line-height:22px}._6Du3AG_popupDialog._6Du3AG_popupDialog{width:min(460px,100%)}._6Du3AG_popupRows{flex-direction:column;gap:10px;width:100%;display:flex}._6Du3AG_popupRow{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;justify-content:space-between;align-items:center;gap:16px;min-width:0;padding:14px 16px;transition:border-color .16s,background .16s;display:flex}._6Du3AG_popupRow:hover{border-color:var(--dsw-alias-label-dimmed)}._6Du3AG_done{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}._6Du3AG_done:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}._6Du3AG_error{color:var(--dsw-alias-state-error-primary);padding:10px 0 2px;font-size:12px;line-height:17px}._6Du3AG_pluginModal._6Du3AG_pluginModal{width:min(560px,100%)}._6Du3AG_pluginList{flex-direction:column;gap:12px;width:100%;display:flex}._6Du3AG_pluginTopicBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:pointer;border-radius:8px;padding:6px 12px;font-size:12px;line-height:18px}._6Du3AG_pluginTopicBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}._6Du3AG_pluginTopicBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}._6Du3AG_pluginEmpty{color:var(--dsw-alias-label-tertiary);padding:20px 2px;font-size:12px;line-height:18px}._6Du3AG_pluginEntries{flex-direction:column;gap:10px;display:flex}._6Du3AG_pluginEntry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:12px;flex-direction:column;gap:4px;padding:12px;display:flex}._6Du3AG_pluginEntryHead{justify-content:space-between;align-items:center;gap:12px;display:flex}._6Du3AG_pluginEntryActions{flex:none;align-items:center;gap:6px;display:inline-flex}._6Du3AG_pluginJumpBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}._6Du3AG_pluginJumpBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}._6Du3AG_pluginJumpBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}._6Du3AG_pluginName{appearance:none;min-width:0;font:inherit;color:var(--dsw-alias-label-primary);text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;background:0 0;border:0;padding:0;font-size:13px;font-weight:600;line-height:20px;text-decoration:none;overflow:hidden}._6Du3AG_pluginName:hover{color:var(--dsw-alias-button-primary-fill);text-decoration:underline}._6Du3AG_pluginDesc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}._6Du3AG_pluginInstall{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);white-space:nowrap;border-radius:8px;padding:6px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:16px;display:block;overflow-x:auto}._6Du3AG_pluginCopyBtn{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}._6Du3AG_pluginCopyBtn:hover{background:var(--dsw-alias-button-primary-hover)}._6Du3AG_pluginCopyBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}@media (prefers-reduced-motion:reduce){._6Du3AG_card,._6Du3AG_switchTrack,._6Du3AG_switchThumb{transition:none}}";
		const tagId$1 = "starpivot/dsh-better-sidebar/SideCardSection.module.css";
		...
		var SideCardSection_module_css_default = {
			"control": "_6Du3AG_control",
			"pluginEntryActions": "_6Du3AG_pluginEntryActions",
			"switchTrack": "_6Du3AG_switchTrack",
			"suffix": "_6Du3AG_suffix",
			"pluginEntryHead": "_6Du3AG_pluginEntryHead",
			"pluginInstall": "_6Du3AG_pluginInstall",
			"title": "_6Du3AG_title",
			"pluginTopicBtn": "_6Du3AG_pluginTopicBtn",
			"cardMain": "_6Du3AG_cardMain",
			"rowGear": "_6Du3AG_rowGear",
			"cardCheck": "_6Du3AG_cardCheck",
			"popupRows": "_6Du3AG_popupRows",
			"cardOn": "_6Du3AG_cardOn",
			"count": "_6Du3AG_count",
			"group": "_6Du3AG_group",
			"cardIconChip": "_6Du3AG_cardIconChip",
			"row": "_6Du3AG_row",
			"cardDesc": "_6Du3AG_cardDesc",
			"switchInput": "_6Du3AG_switchInput",
			"rowText": "_6Du3AG_rowText",
			"typedInput": "_6Du3AG_typedInput",
			"typedInputNumber": "_6Du3AG_typedInputNumber",
			"selectAnchorText": "_6Du3AG_selectAnchorText",
			"popupDialog": "_6Du3AG_popupDialog",
			"pluginList": "_6Du3AG_pluginList",
			"pluginDesc": "_6Du3AG_pluginDesc",
			"selectOptionIcon": "_6Du3AG_selectOptionIcon",
			"pluginEmpty": "_6Du3AG_pluginEmpty",
			"selectAnchor": "_6Du3AG_selectAnchor",
			"intro": "_6Du3AG_intro",
			"grid": "_6Du3AG_grid",
			"percentInput": "_6Du3AG_percentInput",
			"card": "_6Du3AG_card",
			"cardTitle": "_6Du3AG_cardTitle",
			"selectOptionText": "_6Du3AG_selectOptionText",
			"pluginModal": "_6Du3AG_pluginModal",
			"pluginName": "_6Du3AG_pluginName",
			"cardWithGear": "_6Du3AG_cardWithGear",
			"selectAnchorIcon": "_6Du3AG_selectAnchorIcon",
			"cardTop": "_6Du3AG_cardTop",
			"groupHeading": "_6Du3AG_groupHeading",
			"selectOption": "_6Du3AG_selectOption",
			"popupRow": "_6Du3AG_popupRow",
			"done": "_6Du3AG_done",
			"pluginEntries": "_6Du3AG_pluginEntries",
			"desc": "_6Du3AG_desc",
			"pluginEntry": "_6Du3AG_pluginEntry",
			"section": "_6Du3AG_section",
			"pluginJumpBtn": "_6Du3AG_pluginJumpBtn",
			"switch": "_6Du3AG_switch",
			"cardGear": "_6Du3AG_cardGear",
			"error": "_6Du3AG_error",
			"addCard": "_6Du3AG_addCard",
			"pluginCopyBtn": "_6Du3AG_pluginCopyBtn",
			"switchThumb": "_6Du3AG_switchThumb"
		};
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Volumes/ExternalData/Projects/dsh-plugins-public/packages/better-sidebar/src/client/layout.css.mjs
		...
		const tagId = "starpivot/dsh-better-sidebar/layout.css";

...
*/
function isWithinAny(bases, target, platform = process.platform) {
...
*/
function uniqueRoots(cwd, folders = []) {
	const roots = [cwd];
...
*/
async function searchFilesInRoots(roots, query, opts = {}) {
...
function absoluteSearchMatch(root, rel) {
...
function isPluginRoot(dir) {
	...
	try {
		return JSON.parse(readFileSync(file, "utf8")).name === "@starpivot/dsh-better-sidebar";
	} catch {
		return false;
	}
}
...
const name = "@starpivot/dsh-better-sidebar";
...
*/
function workspaceFoldersOf(ctx, sessionId, cwd) {
	const lookup = ctx.get;
	const registry = typeof lookup === "function" ? lookup.call(ctx, "workspaceRegistry") : void 0;
	...
	return byPath === void 0 ? [] : [...byPath.folders];
}
/**
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (!isWithinAny(uniqueRoots(cwd, workspaceFoldersOf(ctx, sessionId, cwd)), path)) throw new SidebarError("fs-error", "media path outside the workspace folders", 403);
				const info = await stat(path);
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (!isWithinAny(uniqueRoots(cwd, workspaceFoldersOf(ctx, sessionId, cwd)), absolute)) throw new SidebarError("fs-error", "html path outside the workspace folders", 403);
